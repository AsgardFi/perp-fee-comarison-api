...
import { FeeComparison } from './modal';
...
const fileContent = fs.readFileSync('./scripts/SOL-PERP-funding-rate.csv', { encoding: 'utf-8' });
  ...
  for (const row of records) {
    try {
      ...
      const result = await FeeComparison.findOneAndUpdate(
        ...
        {
          $set: {
            'drift.SOLPerp.driftHourlyFunding': fundingRate,
          },
        },
        { new: true }
      );
